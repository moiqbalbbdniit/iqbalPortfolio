...
const ContactMe: React.FC = () => {
  ...
  const processCommand = () => {
    ...
    const pushRegex = /^git push origin iqbal$/;
 ...
 const handleSubmit = async () => {
  ...
  try {
    const res = await fetch('https://api.web3forms.com/submit', {
      ...
      body: JSON.stringify({
        access_key: 'ecd3ebc0-4e2c-4092-a9bb-685daedd7991',
    ...
    console.log('Web3Forms Response:', result); // 👈 See detailed error

    if (result.success) {
      setFormStatus('Message sent successfully!');
      setFormData({ name: '', email: '', message: '' });
    } else {
      setFormStatus(`Failed: ${result.message || 'Please try again.'}`);
    }
  } catch (error) {
  ...
  return (
    <div className={`flex flex-col items-center justify-center min-h-screen ${theme === 'dark' ? 'bg-transparent  text-white' : 'bg-transparent text-black'}`}>
      ...
      >
        <motion.a
          href="mailto:iqbal.engineer.it@gmail.com"
     ...
     git add . "your message for me" && git commit -m "your mail id"
      ...
      >
        <div className="mb-4">
          ...
          <p className="text-green-500">$ git push origin iqbal</p>
        </div>
